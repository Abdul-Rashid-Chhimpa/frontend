...
const API_BASE = "https://backend-3-axez.onrender.com";
...
const getStepIndex = (status) => {
  ...
  const idx = STEPPER_STEPS.findIndex(
...
const getStepTime = (stepKey, order, currentStatus) => {
  ...
  const matchingHistory = history
    ...
    .filter(
      (entry) =>
        entry.status === stepKey &&
        entry.at
    );

  if (matchingHistory.length > 0) {
    return formatDateTime(
      matchingHistory[matchingHistory.length - 1].at
    );
  }

  // Pending = order creation time
  if (stepKey === "Pending") {
    return formatDateTime(
      order?.createdAt ||
        order?.orderDate ||
...
const MyOrders = () => {
  ...
  // ----------------------------------------------------
  // IMPORTANT:
  // Stores the status we previously saw for each order.
  // Used to detect:
  //
  // Pending -> Confirmed
  //
  ...
  const previousStatusesRef =
    useRef({});

  // Prevents popup during initial loading
  const firstFetchDoneRef =
    useRef(false);
  ...
  const companyDetails = {
    ...
    email:
      "pedwalifecreation4u@gmail.com",

    phone:
      "+91 9887663598, +91 7412945826",
  ...
  const fetchOrders = async (
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Error fetching products for GST:",
          prodErr
        );
      }
  ...
  useEffect(() => {
    ...
    // --------------------------------------------------
    // Check admin changes every 10 seconds
    // --------------------------------------------------
  ...
  // ====================================================
  // DELETE ORDER FROM USER DASHBOARD
  // ====================================================
  ...
  const handleDeleteOrder =
    async (orderId) => {
      ...
      try {
        ...
        const { data } =
          await axios.put(
            `${API_BASE}/api/orders/user-delete/${orderId}`
          );
        ...
        toast.error(
          error.response?.data
            ?.message ||
            "Server Error: Unable to remove order"
        );
  ...
  const handleDownloadInvoice =
    (order) => {
      ...
      if (!isUnlocked) {
        ...
        toast.error(
          "Invoice will unlock once the admin confirms your order!"
        );
      ...
      if (!printWindow) {
        ...
        toast.error(
          "Popup blocked! Please allow popups to view/download invoice."
        );
          ...
          <div class="invoice-container">
            ...
            <table class="invoice-table">
              ...
              <tbody>
                ...
                <tr class="total-row">
                  ...
                  <td style="text-align:right;">
                    Rs.
                    ${calculatedTotalGstAmount.toFixed(
                      2
                    )}
                  </td>
            ...
            <div class="footer-grid">
              ...
              <div class="footer-left">
                ...
                1. Goods once sold will not
                be taken back.

                <br />

                2. 18% per month will be
                charged on the bill if not
                made within 30 days.

                <br />

                3. Subjected to
                'RAJASTHAN' jurisdiction only.
